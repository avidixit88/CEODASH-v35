...
def _clinical_bucket_lines(clinical, bucket: str, limit: int = 2) -> list[str]:
    ...
    for signal in signals:
        if getattr(signal, "bucket", "") != bucket:
            continue
        if bucket == "value":
            finding = str(getattr(signal, "finding", "") or "").strip()
            value = str(getattr(signal, "value", "") or "").strip()
            text = (finding + " " + value).strip() if finding else value
        else:
...
def _clinical_status_line(clinical) -> str | None:
    ...
    if total <= 0:
        return f"ClinicalTrials.gov live pull status: {status}. No usable records were returned, so the system preserved the run and captured diagnostics."
    return f"ClinicalTrials.gov live pull: {total} relevant records across {lanes} lanes, including {active} active/recruiting-style studies."
...
def render_premium_executive_summary(results) -> None:
    ...
    if synthesis is not None:
        ...
        if gaps is not None and not gaps.empty:
            top = gaps.iloc[0]
            position_lines.append(f"Top peer read-through: {top.get('Ticker', 'Peer')} is creating the largest competitive gap in this run.")
    ...
    cards = [
        ("01", "What new information did we find?", fresh_lines, "New intelligence elevated above the raw market data."),
        ("02", "Why is that information valuable?", value_lines, "Strategic implication, not just another observation."),
        ("03", "What trends can be inferred?", trend_lines, "Patterns forming across peers, lanes, and visibility."),
        ("04", "How is NXTC positioned?", position_lines, "Company behavior versus XBI, QQQ, and relevant peers."),
    ]
