...
if st.session_state.analysis_started and st.session_state.results is not None:
    ...
    if active_page == "Fresh Intelligence":
        ...
        st.caption("External strategic signals synthesized into NextCure-specific relevance. ClinicalTrials.gov is now a live pull on each analysis run; raw records are kept as drill-down evidence, not executive clutter.")
        ...
        if clinical is not None:
            ...
            with c2:
                st.metric("Records", int(getattr(clinical, "total_trials", 0) or 0))
            with c3:
            ...
            if trial_table is not None and not trial_table.empty:
                with st.expander("ClinicalTrials.gov normalized records from this run"):
                    st.dataframe(trial_table, use_container_width=True, hide_index=True)

            errors = getattr(clinical, "source_errors", []) or []
            if errors:
